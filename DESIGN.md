...
### Settings
- **Style:** Stock grouped `Form`, no customization. Sections: **Units** (segmented `Picker`, kg/lb, applies live including mid-workout), **Speech** (status row — Granted / Denied / Not determined / Unavailable on this device — plus a conditional "Open iOS Settings" button shown for Denied only), **Exercises** (a `NavigationLink` row to the library, trailing count), **Export** ("Export Training History" — a `confirmationDialog` picks JSON or CSV, then the system share sheet; disabled with an explanatory footer when no completed workout exists), **Apple Health** (a "Write workouts to Apple Health" toggle that, on enable, fires the system Health authorization sheet; when authorization is denied or HealthKit is unavailable it shows a status line and, for Denied only, a conditional "Open iOS Settings" button reusing the speech-recovery pattern), **Data** (destructive-role "Delete All Workout Data", disabled while a workout is open with an explanatory footer, guarded by a `confirmationDialog`).
