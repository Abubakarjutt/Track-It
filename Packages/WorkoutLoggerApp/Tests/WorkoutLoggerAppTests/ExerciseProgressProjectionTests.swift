...
struct ExerciseProgressProjectionTests {
    ...
    private func session(_ t: TimeInterval, volume: Double, reps: Int, setCount: Int = 1,
                         top: Double?, e1rm: Double?) -> ExerciseSession {
        ExerciseSession(date: Date(timeIntervalSince1970: t), volumeKilograms: volume,
                        workingReps: reps, workingSetCount: setCount,
                        topSetLoadKilograms: top, bestEstimatedOneRepMaxKilograms: e1rm)
     }
    ...
    func empty() {
        let p = ExerciseProgressProjection(progress: ExerciseProgress(sessions: []), unit: .kilograms)
         ...
         #expect(p.setVolumeSeries.isEmpty)
    ...
    func bodyweightSessionSparseSeries() {
        let progress = ExerciseProgress(sessions: [
            session(1_000, volume: 0, reps: 30, setCount: 2, top: nil, e1rm: nil),
            session(2_000, volume: 0, reps: 33, setCount: 3, top: nil, e1rm: nil),
         ])
        let p = ExerciseProgressProjection(progress: progress, unit: .kilograms)
         #expect(p.loadSeries.isEmpty)
         #expect(p.estimatedOneRepMaxSeries.isEmpty)
         #expect(p.volumeSeries.map(\.value) == [0, 0])
        // set volume is independent of tonnage: a zero-kg bodyweight session still counts
         #expect(p.setVolumeSeries.map(\.value) == [2, 3])
         #expect(p.comparison?.setVolumeDelta == 1)
